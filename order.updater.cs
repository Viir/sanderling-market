...
IWindow ModalUIElement =>
 Measurement ? .EnumerateReferencedUIElementTransitive() ? .OfType < IWindow > () ? .Where(window => window ? .isModal ? ? false) ? .OrderByDescending(window => window ? .InTreeIndex ? ? int.MinValue) ? .FirstOrDefault();
...
void CloseModalUIElementYes() {
 var ButtonClose =
  ModalUIElement ? .ButtonText ? .FirstOrDefault(button => (button ? .Text).RegexMatchSuccessIgnoreCase("yes"));
   ...
   if (orderEntry.OrderComplete == false) {
    ...
    if (timeToCheck) {
     ...
     if (foundOrder == false) {
      ...
      while (Measurement ? .WindowRegionalMarket ? [0] ? .SelectedItemTypeDetails ? .MarketData ? .BuyerView == null &&
       Measurement ? .WindowRegionalMarket ? [0] ? .SelectedItemTypeDetails ? .MarketData ? .SellerView == null) {
